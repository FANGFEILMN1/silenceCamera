...
public class CameraView extends SurfaceView implements CameraOperation{
	...
	public CameraView(Context context, AttributeSet attrs) {
		...
		getHolder().addCallback(callback);
		openCamera();
		if (null == mCamera) throw new java.lang.RuntimeException();
		mCamera.startPreview();
		Camera.Parameters parameters = mCamera.getParameters();
		//mCamera.setDisplayOrientation(180);//预览转90°
...
/*		mIsFrontCamera=!mIsFrontCamera;
	...
	private void setCameraParameters(){
		...
		if (sizeList.size()>0) {
			...
			int a = max_size.width*max_size.height;
			//Toast.makeText(getContext(), "画面大小:"+max_size.width+"*"+max_size.height, Toast.LENGTH_SHORT).show();
			//Log.e(TAG, "画面大小:"+a);
		}
		//设置图片格式
	...
	public void forceToFocus(AutoFocusCallback callback)
	{
		mCamera.autoFocus(callback);
	}
	public void releaseCamera()
	{
		if(mMediaRecorder!=null){
			...
			mMediaRecorder.release();
			mMediaRecorder=null;
			Log.e(TAG, "mMediaRecorder.release");
		}
		if (mCamera != null) {
			mCamera.stopPreview();
			mCamera.release();
			mCamera = null;
			Log.e(TAG, "mCamera.release");
		}
